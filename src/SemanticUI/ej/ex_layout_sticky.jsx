...
export default function StickyLayout() {
  ...
  const handleOverlayRef = (c) => {
    ...
    const { overlayRect } = state

    if (!overlayRect) {
      setState({ ...state, overlayRect: _.pick(c.getBoundingClientRect(), 'height', 'width') })
    }
  }

  const stickOverlay = () => setState({ ...state, overlayFixed: true })

  const stickTopMenu = () => setState({ ...state, menuFixed: true })

  const unStickOverlay = () => setState({ ...state, overlayFixed: false })

  const unStickTopMenu = () => setState({ ...state, menuFixed: false })

  const { menuFixed, overlayFixed, overlayRect } = state
